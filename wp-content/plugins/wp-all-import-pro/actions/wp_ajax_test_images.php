	...
	if ( ! @is_writable($targetDir) )
	{
		...
		if ( 'no' == $download ){		
			...
			if ((int)$success_images === 1)
				$success_msg = sprintf(__('%d image was successfully retrieved from `%s/wpallimport/files`', 'wp_all_import_plugin'), $success_images, preg_replace('%.*/wp-content%', '/wp-content', $wp_uploads['basedir']));		
			elseif ((int)$success_images > 1)
				$success_msg = sprintf(__('%d images were successfully retrieved from `%s/wpallimport/files`', 'wp_all_import_plugin'), $success_images, preg_replace('%.*/wp-content%', '/wp-content', $wp_uploads['basedir']));		
		}
		else {
