...
<form name="settings" method="post" action="<?php echo $this->baseUrl ?>" class="settings">
	...
	<h3><?php _e('Cron Imports', 'pmxi_plugin') ?></h3>
	
	<table class="form-table">
		<tbody>
			<tr>
				<th scope="row"><label><?php _e('Secret Key', 'pmxi_plugin'); ?></label></th>
				<td>
					<input type="text" class="regular-text" name="cron_job_key" value="<?php echo esc_attr($post['cron_job_key']); ?>"/>
					<p class="description"><?php _e('Changing this will require you to re-create your existing cron jobs.', 'pmxi_plugin'); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><label><?php _e('Cron Processing Time Limit', 'pmxi_plugin'); ?></label></th>
				<td>
					...
					<p class="description"><?php _e('Leave blank to use your server\'s limit on script run times.', 'pmxi_plugin'); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><label><?php _e('Cron Sleep', 'pmxi_plugin'); ?></label></th>
				<td>
					<input type="text" class="regular-text" name="cron_sleep" value="<?php echo esc_attr($post['cron_sleep']); ?>"/>
					<p class="description"><?php _e('Sleep the specified number of seconds between each post created, updated, or deleted with cron. Leave blank to not sleep. Only necessary on servers  that are slowed down by the cron job because they have very minimal processing power and resources.', 'pmxi_plugin'); ?></p>
				</td>
			</tr>
		</tbody>
	</table>	
	...
	<table class="form-table">
		<tbody>
			...
			<tr>
				<th scope="row"><label><?php _e('Add Port To URL', 'pmxi_plugin'); ?></label></th>
				<td>
					<input type="text" class="regular-text" name="port" value="<?php echo esc_attr($post['port']); ?>"/>
					<p class="description"><?php _e('Specify the port number to add if you\'re having problems continuing to Step 2 and are running things on a custom port. Default is blank.', 'pmxi_plugin'); ?></p>
				</td>
			</tr>		
