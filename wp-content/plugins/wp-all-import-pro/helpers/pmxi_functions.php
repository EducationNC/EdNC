		...
		function pmxi_copy_url_file($filePath, $detect = false, $targetDir = false){
			...
			if (empty($type)){
				...
				$headers = get_headers($filePath, 1);
	   			...
	   			if ( ! empty($headers['Content-Type']) and empty($type)){
	   				if (is_array($headers['Content-Type'])){
	   					foreach ($headers['Content-Type'] as $key => $ct) {
	   						if (strpos($ct, 'xml') !== false){
			   					$type = 'xml';
			   				}
			   				elseif (strpos($ct, 'csv') !== false){
			   					$type = 'csv';
			   				}
			   				else{
								$type = false;	
		   				...
		   				elseif (strpos($headers['Content-Type'], 'csv') !== false){
		   					$type = 'csv';
		   				}
		   				else{
							$type = false;	
   			...
   			if ( $headers and ! empty($headers['Content-Encoding']) and $headers['Content-Encoding'] == 'gzip'){
   				$file = @fopen($filePath);
   			}
   			else{
	   		...
	   		if (is_resource($file)){   				   			
				...
				while ( ! @feof($file) ) {
					...
					if (!$type and $first_chunk and (strpos($chunk, "<?") !== false or strpos($chunk, "<rss") !== false)) $type = 'xml'; elseif (!$type and $first_chunk) $type = 'csv'; // if it's a 1st chunk, then chunk <? symbols to detect XML file
					$first_chunk = false;
				 	@fwrite($fp, $chunk);		 	
				}
				@fclose($file);
		   	...
		   	if ( ! file_exists($localPath) ) {
		   		...
		   		if ( ! is_wp_error($request) ){
			   		...
			   		if ( ! $type ){	   			
			   			...
			   			if ( ! $type ){
			   				if ( ! empty($headers['Content-Encoding']) and $headers['Content-Encoding'] == 'gzip'){
							...
							while (!@feof($file)) {
								$chunk = @fread($file, 1024);					
								if (strpos($chunk, "<?") !== false or strpos($chunk, "<rss") !== false) $type = 'xml'; else $type = 'csv'; // if it's a 1st chunk, then chunk <? symbols to detect XML file					
							 	break;		 	
							}
							@fclose($file);	
						}
